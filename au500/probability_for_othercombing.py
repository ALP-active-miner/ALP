"""
Finds usage of API in external projects. (graphs mined in ALPPipelineCombOtherProjectForAPIUsageGraphBuilder.java)
Predicts if each usage is a misuse.
...
if len(test_vector) > 0:
	...
	for model in models:
		...
		for file_id in file_mappings.keys():
			...
			if is_misuse:  #misuse_prob > 0.5:
				...
				# print(test_vector.values[file_id])
	...
	for i, (ans, val) in enumerate(sorted_ans.items()):
		...
		if i > 500: 
			break
	sys.stderr.write(" sorted_ans len is "+ str(len(sorted_ans) ) + "\n")
else:
